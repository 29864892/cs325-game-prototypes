...
BasicGame.BattleTwo = function (game) {
...
BasicGame.BattleTwo.prototype = {

    create: function () {
		this.music = this.add.audio('bBoyMusic');
		this.music.play();
		...
		this.attackButton;
		this.runButton;
		this.EncounterText = this.add.text(50, 370, "A Bottle Boy wants to fight!",{ font: "bold 32px Impact", fill: "#8215C8", boundsAlignH: "center", boundsAlignV: "middle" });
		//player
		...
		//enemy
		this.enemy = this.add.sprite(590, 135, 'bBoy');
		this.eHealth = 500;//00;
		this.enemyName = this.add.text(600,40,"Bottle Boy",{font: "16px Comic Sans MS", fill: "#8215C8", boundsAlignH: "center", boundsAlignV: "middle" });
		this.eHealthText = this.add.text(600,60, "Health: 500",{font: "16px Comic Sans MS", fill: "#8215C8", boundsAlignH: "center", boundsAlignV: "middle" });
		this.eDarknessText = this.add.text(600,80, "Energy: 0",{font: "16px Comic Sans MS", fill: "#8215C8", boundsAlignH: "center", boundsAlignV: "middle" });
		this.isLowE = false;
		this.isTurnE = false;
		this.damageM = 1.20;
		this.eLowSprite = this.add.sprite(590, 135, 'bBoyF');
    ...
    update: function () {
		...
		this.eDarknessText.setText("Energy: " + this.oblivion);
		//health conditions
		...
		if(this.eHealth < 200 && this.isLowE == false){
			//change sprite
		...
		if(this.eHealth <= 0 && this.continueButton.visible == false){
			this.eHealth = 0;
			if(this.end == false){
				this.EncounterText.setText('Bottle Boy is dehydrated!' ,{ font: "bold 32px Impact", fill: "#8215C8", boundsAlignH: "center", boundsAlignV: "middle" });
				this.messageUI();
				this.end = true;
				this.won = true;
		...
		if(this.chargeVal >= 10){
			if(!this.isLow && this.chargePrompt){
				this.EncounterText.setText('Bapple Ifighter released its energy, \nbut it had no effect!' ,{ font: "bold 32px Impact", fill: "#8215C8", boundsAlignH: "center", boundsAlignV: "middle" });
				this.messageUI();
				this.resetUI();
				this.lowPrompt = false;
				this.chargePrompt = false;
			}
	...
	smartPunchAttack: function(){
		...
		this.sPDamage = Phaser.Math.roundTo(this.sPDamage,-1);
		this.hideAttackUI();
		...
		this.eHealth = Phaser.Math.roundTo(this.eHealth,-1);
		if(this.eHealth < 0){
			this.eHealth = 0;
		}
		this.messageUI();
	...
	//decrease enemy attack completely for 5 turns
	wrongNumberAttack: function(){
		this.hasWall = true;
		this.wallCounter = 0;
		this.damageM = 0;
		this.EncounterText.setText('Bapple Ifighter called Donald Trump and was \n sent a tactical wall!' ,{ font: "bold 32px Impact", fill: "#8215C8", boundsAlignH: "center", boundsAlignV: "middle" });
		this.hideAttackUI();
		this.messageUI();
		this.resetUI();
	},
	...
	enemyTurn: function(){
		this.isTurnE = true;
		this.damage = this.rnd.integerInRange(16, 20)*this.damageM;
		this.damage = Phaser.Math.roundTo(this.damage,-1);
		if(this.oblivion == 34 && this.eHealth > 0){
			this.EncounterText.setText('Bottle Boy is powered up!');
			this.messageUI();
			this.isTurn = true;
		}
		else{
			if(this.drink && this.eHealth < 250){//heal if below half hp once
				this.EncounterText.setText('Bottle Boy absorbed water, healing the damage!');
				this.eHealth = 500;
				this.messageUI();
				this.isTurn = true;
				this.drink = false;
				...
				this.EncounterText.setText('Bottle Boy sprayed water, dealing ' + this.damage + ' damage!');
				this.pHealth -= this.damage;
				this.pHealth = Phaser.Math.roundTo(this.pHealth,-1);
				this.messageUI();
				this.isTurn = true;
				this.attackM -= .10;//decrease attack after each attack
			}
		...
		if(this.oblivion == 35 && this.eHealth > 0){
			this.pHealth = 0;
			this.EncounterText.setText('Bottle Boy sent Bapple Ifighter to the Shadow Realm!');
			this.messageUI();
			this.end = true;
		}
		if(this.hasWall){
			this.wallCounter++;
			if(this.wallCounter == 6){
				this.damageM = 1.20;
				console.log('attack reset!');
			}
		}
		console.log(this.oblivion);
		this.oblivion++;
	},
